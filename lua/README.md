...
The Lua SDK for the DataDragon API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Champion(nil):load({ id = "example_id" }, nil)
...
local result, err = client:DataDragon(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── data-dragon_sdk.lua    -- Main SDK module
...
The main module (`data-dragon_sdk`) exports the SDK constructor

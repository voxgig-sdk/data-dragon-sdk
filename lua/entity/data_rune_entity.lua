...
local DataRuneEntity = {}
DataRuneEntity.__index = DataRuneEntity


function DataRuneEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DataRuneEntity)
  self._name = "data_rune"
  self._client = client
...
function DataRuneEntity:get_name()
  return self._name
end


function DataRuneEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DataRuneEntity.new(self._client, opts)
end
...
function DataRuneEntity:mark_deleted()
  self._deleted = true
end


function DataRuneEntity:deleted()
  return true == self._deleted
end


function DataRuneEntity:data_set(args)
  if args ~= nil then
...
function DataRuneEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DataRuneEntity:match_set(args)
  if args ~= nil then
...
function DataRuneEntity:match_get()
...
function DataRuneEntity:stream(action, args, callopts)
...
---@param reqmatch DataRuneLoadMatch
---@param ctrl? table
---@return DataRune
---@return string? err
function DataRuneEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DataRuneEntity:_run_op(ctx, post_done)
  local utility = self._utility

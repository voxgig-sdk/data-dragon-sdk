...
local DataItemEntity = {}
DataItemEntity.__index = DataItemEntity


function DataItemEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DataItemEntity)
  self._name = "data_item"
  self._client = client
...
function DataItemEntity:get_name()
  return self._name
end


function DataItemEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DataItemEntity.new(self._client, opts)
end
...
function DataItemEntity:mark_deleted()
  self._deleted = true
end


function DataItemEntity:deleted()
  return true == self._deleted
end


function DataItemEntity:data_set(args)
  if args ~= nil then
...
function DataItemEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DataItemEntity:match_set(args)
  if args ~= nil then
...
function DataItemEntity:match_get()
...
function DataItemEntity:stream(action, args, callopts)
...
function DataItemEntity:_run_op(ctx, post_done)
  local utility = self._utility

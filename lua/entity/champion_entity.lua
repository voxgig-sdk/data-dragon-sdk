...
local ChampionEntity = {}
ChampionEntity.__index = ChampionEntity


function ChampionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ChampionEntity)
  self._name = "champion"
  self._client = client
...
function ChampionEntity:get_name()
  return self._name
end


function ChampionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ChampionEntity.new(self._client, opts)
end
...
function ChampionEntity:mark_deleted()
  self._deleted = true
end


function ChampionEntity:deleted()
  return true == self._deleted
end


function ChampionEntity:data_set(args)
  if args ~= nil then
...
function ChampionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ChampionEntity:match_set(args)
  if args ~= nil then
...
function ChampionEntity:match_get()
...
function ChampionEntity:stream(action, args, callopts)
...
---@param reqmatch ChampionLoadMatch
---@param ctrl? table
---@return Champion
---@return string? err
function ChampionEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function ChampionEntity:_run_op(ctx, post_done)
  local utility = self._utility

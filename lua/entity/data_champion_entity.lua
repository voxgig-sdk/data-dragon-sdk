...
local DataChampionEntity = {}
DataChampionEntity.__index = DataChampionEntity


function DataChampionEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DataChampionEntity)
  self._name = "data_champion"
  self._client = client
...
function DataChampionEntity:get_name()
  return self._name
end


function DataChampionEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DataChampionEntity.new(self._client, opts)
end
...
function DataChampionEntity:mark_deleted()
  self._deleted = true
end


function DataChampionEntity:deleted()
  return true == self._deleted
end


function DataChampionEntity:data_set(args)
  if args ~= nil then
...
function DataChampionEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DataChampionEntity:match_set(args)
  if args ~= nil then
...
function DataChampionEntity:match_get()
...
function DataChampionEntity:stream(action, args, callopts)
...
function DataChampionEntity:_run_op(ctx, post_done)
  local utility = self._utility

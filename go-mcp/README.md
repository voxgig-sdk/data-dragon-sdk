...
[MCP](https://modelcontextprotocol.io) server exposing the DataDragon SDK as
two agent tools — `data-dragon_list` and `data-dragon_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/data-dragon-mcp)
make build

# 2. Provide credentials via the environment
export DATA_DRAGON_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user data-dragon \
  -- /absolute/path/to/data-dragon-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./data-dragon-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// data-dragon_list: first page of records
{ "entity": "version" }
{ "entity": "version", "query": { } }

// data-dragon_load: one record by id
{ "entity": "champion", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/data-dragon-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user data-dragon \
     -- "$PWD"/dist/*/data-dragon-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `data-dragon_list` and `data-dragon_load` tools now appear
   in new sessions. Ask the agent to *"list version using data-dragon"*
   and it calls `data-dragon_list` with `{"entity":"version"}`.
...
```sh
export DATA_DRAGON_APIKEY=sk_live_xxx            # API key
export DATA_DRAGON_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `data-dragon_list` tool
...
### Call the `data-dragon_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `data-dragon_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `data-dragon_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DATA_DRAGON_APIKEY` | API key sent with every request. |
| `DATA_DRAGON_BASE` | Optional override of the API base URL. |

### Entities

The 8 entities valid as the `entity` argument:

champion | data_champion | data_item | data_rune | dragontail_versiontgz | item | region | version

### Smoke test via HTTP (raw JSON-RPC)

```sh
./data-dragon-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"data-dragon_load","arguments":{"entity":"champion","query":{"id":1}}}}'
```

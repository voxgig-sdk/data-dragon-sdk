...
type Args struct {
	Entity string         `json:"entity" jsonschema:"champion | data_champion | data_item | data_rune | dragontail_versiontgz | item | region | version"`
	Query  map[string]any `json:"query,omitempty" jsonschema:"optional match map e.g. {\"id\":1} for load, omit for list"`
}

func registerTools(server *mcp.Server, client *sdk.DataDragonSDK) {
	mcp.AddTool(server, &mcp.Tool{
		Name: "data-dragon_list",
		Description: "List records from DataDragon. " +
	...
	mcp.AddTool(server, &mcp.Tool{
		Name: "data-dragon_load",
		Description: "Load a single record from DataDragon. " +
...
func runOp(client *sdk.DataDragonSDK, op string, args Args) (*mcp.CallToolResult, any, error) {
...
func entityFor(client *sdk.DataDragonSDK, name string) (sdk.DataDragonEntity, error) {
	switch strings.ToLower(name) {
	case "champion":
		return client.Champion(nil), nil
	case "data_champion":
		return client.DataChampion(nil), nil
	case "data_item":
		return client.DataItem(nil), nil
	case "data_rune":
		return client.DataRune(nil), nil
	case "dragontail_versiontgz":
		return client.DragontailVersiontgz(nil), nil
	case "item":

...
boru-driven command-line client **and** interactive REPL for the DataDragon
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/data-dragon-cli)
make build

# 2. See usage (words, entities, env vars)
./data-dragon-cli --help

# 3. Provide credentials once, via the environment
export DATA_DRAGON_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./data-dragon-cli load 1 champion            # {id:1} shorthand
./data-dragon-cli load '{id:1}' champion       # explicit match map

# 5. Override the API base URL for a single call
DATA_DRAGON_BASE=https://api.example.com ./data-dragon-cli load 1 champion

# 6. No arguments -> interactive REPL
./data-dragon-cli
data-dragon> load 1 champion
data-dragon> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/data-dragon-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/data-dragon-cli load 1 champion
   ```
...
```sh
./data-dragon-cli load 1 champion          # scalar shorthand for {id:1}
./data-dragon-cli load '{id:1}' champion     # explicit match map
```
...
./data-dragon-cli load 1 champion
```
...
Run with no arguments to open a REPL (prompt `data-dragon>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `champion`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/data-dragon-cli`. |
...
(`./data-dragon-cli <expr>`) and interactively (the REPL), and expressions compose the

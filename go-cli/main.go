// Command DataDragon-cli is a CLI + REPL for the DataDragon SDK,
...
const prompt = "data-dragon"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "champion data_champion data_item data_rune dragontail_versiontgz item region version"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DATA_DRAGON_APIKEY (api key), DATA_DRAGON_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DATA_DRAGON_APIKEY carries the API key and
	// DATA_DRAGON_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DATA_DRAGON_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DATA_DRAGON_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDataDragonSDK(opts)

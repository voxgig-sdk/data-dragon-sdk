...
module DataDragonUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DataDragonResponse.new({})
    ...
    if fetch_err
      ...
      response = DataDragonResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DataDragonResponse.new(fetched)
    else

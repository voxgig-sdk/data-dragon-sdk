...
class FtCtrl extends DataDragonControl
{
...
class FtClient
{
    ...
    private DataDragonUtility $utility;

    public function __construct(DataDragonUtility $utility, array $options)
    {
    ...
    public function get_utility(): DataDragonUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DataDragonContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DataDragonContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DataDragonContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DataDragonConfig::make_feature($name);
    ...
    private static function buildUrl(DataDragonSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DataDragonError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DataDragonResponse($fetched);
        }
    ...
    private function fail(DataDragonContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DataDragonContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

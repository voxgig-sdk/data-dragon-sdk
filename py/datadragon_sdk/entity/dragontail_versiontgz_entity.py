...
from datadragon_sdk.datadragon_types import (
...
class DragontailVersiontgzEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DragontailVersiontgzEntity(self._client, opts)
    ...
    def data_get(self) -> DragontailVersiontgz:
    ...
    def match_get(self) -> DragontailVersiontgz:
    ...
    def load(self, reqmatch=None, ctrl=None) -> DragontailVersiontgz:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.DragontailVersiontgz().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}

...
class TestDataItemDirect:

    def test_should_direct_load_data_item(self):
        setup = _data_item_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-data_item", "live" if setup["live"] else "unit")
        if _skip:
...
def _data_item_direct_setup(mockres):

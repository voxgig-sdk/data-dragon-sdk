...
class TestDataRuneEntity:

    def test_should_create_instance(self):
        testsdk = DataDragonSDK.test(None, None)
        ent = testsdk.DataRune(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _data_rune_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "data_rune." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set DATA_DRAGON_TEST_DATA_RUNE_ENTID JSON to run live")
        client = setup["client"]

        # Bootstrap entity data from existing test data.
        data_rune_ref01_data_raw = vs.items(helpers.to_map(
            vs.getpath(setup["data"], "existing.data_rune")))
        data_rune_ref01_data = None
        if len(data_rune_ref01_data_raw) > 0:
...
def _data_rune_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/data_rune/DataRuneTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("DATA_DRAGON_TEST_DATA_RUNE_ENTID"))

...
class DataDragonTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            DataDragonTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = DataDragonTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = DataDragonTestRunner.getenv("DATA_DRAGON_TEST_LIVE")
        override = DataDragonTestRunner.getenv("DATA_DRAGON_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = DataDragonTestRunner.getenv(key)
        ...
        explain = DataDragonTestRunner.getenv("DATA_DRAGON_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if DataDragonTestRunner._test_control is not None:
            return DataDragonTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                DataDragonTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            DataDragonTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = DataDragonTestRunner.load_test_control()
...
def load_env_local():
    DataDragonTestRunner.load_env_local()


def env_override(m):
    return DataDragonTestRunner.env_override(m)


def entity_data(v):
    return DataDragonTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return DataDragonTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return DataDragonTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return DataDragonTestRunner.load_test_control()


def live_client_options():
    return DataDragonTestRunner.live_client_options()


def live_delay_ms():
    return DataDragonTestRunner.live_delay_ms()

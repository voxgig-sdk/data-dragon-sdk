...
class TestDataRuneDirect:

    def test_should_direct_load_data_rune(self):
        setup = _data_rune_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-data_rune", "live" if setup["live"] else "unit")
        if _skip:
...
def _data_rune_direct_setup(mockres):

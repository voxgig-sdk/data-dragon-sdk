...
class TestVersionDirect:
    ...
    def test_should_direct_list_version(self):
        setup = _version_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-version", "live" if setup["live"] else "unit")
        if _skip:

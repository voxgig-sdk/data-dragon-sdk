...
class ItemEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DATA_DRAGON_TEST_ITEM_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function item_basic_setup($extra)
{
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["item01", "item02", "item03", "cdn01", "cdn02", "cdn03", "version01"] as $k) {
    ...
    $entid_env_raw = getenv("DATA_DRAGON_TEST_ITEM_ENTID");
    ...
    $idmap_resolved = Helpers::to_map(
        $env["DATA_DRAGON_TEST_ITEM_ENTID"]);

...
class VersionEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = DataDragonSDK::test($seed, null);
        $seen = iterator_to_array($base->Version(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = DataDragonConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = DataDragonSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Version(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set DATA_DRAGON_TEST_VERSION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function version_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("DATA_DRAGON_TEST_VERSION_ENTID");

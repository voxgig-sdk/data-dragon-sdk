...
The Ruby SDK for the DataDragon API. Provides an entity-oriented interface using idiomatic Ruby conventions.
...
```ruby
result, err = client.Champion(nil).load({ "id" => "example_id" }, nil)
raise err if err
puts result
```
...
result, err = client.DataDragon(nil).load(
  { "id" => "test01" }, nil
)
...
```
rb/
├── DataDragon_sdk.rb       -- Main SDK module

...
class DataRuneEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = data_rune_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "data_rune." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    data_rune_ref01_data = nil
    if data_rune_ref01_data_raw.length > 0
      data_rune_ref01_data = Helpers.to_map(data_rune_ref01_data_raw[0][1])
    end

    # LOAD
    data_rune_ref01_ent = client.DataRune(nil)
    data_rune_ref01_match_dt0 = {}
    data_rune_ref01_data_dt0_loaded, err = data_rune_ref01_ent.load(data_rune_ref01_match_dt0, nil)
    assert_nil err
    assert !data_rune_ref01_data_dt0_loaded.nil?

  end
end

def data_rune_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "data_rune", "DataRuneTestData.json")

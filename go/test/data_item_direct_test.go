...
func TestDataItemDirect(t *testing.T) {
	t.Run("direct-load-data_item", func(t *testing.T) {
		setup := data_itemDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-data_item", _mode); _shouldSkip {
			if _reason == "" {
...
type data_itemDirectSetupResult struct {
...
func data_itemDirectSetup(mockres any) *data_itemDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"DATA_DRAGON_TEST_DATA_ITEM_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["DATA_DRAGON_TEST_DATA_ITEM_ENTID"]; ok {
		...
		return &data_itemDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &data_itemDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}

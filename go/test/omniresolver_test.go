...
import (
	...
	omni "github.com/voxgig-sdk/data-dragon-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DataDragonError); is {
				out := map[string]any{
					"name":    "DataDragonError",
...
func omniCtx(arg any, client *sdk.DataDragonSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DataDragonSDK); is {
			client = live

...
func TestItemEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set DATA_DRAGON_TEST_ITEM_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func itemBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"item01", "item02", "item03", "cdn01", "cdn02", "cdn03", "version01"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("DATA_DRAGON_TEST_ITEM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DATA_DRAGON_TEST_ITEM_ENTID"])

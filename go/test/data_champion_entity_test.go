...
func TestDataChampionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		dataChampionRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.data_champion")))
...
func data_championBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DATA_DRAGON_TEST_DATA_CHAMPION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DATA_DRAGON_TEST_DATA_CHAMPION_ENTID"])

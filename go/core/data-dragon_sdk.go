...
func (sdk *DataDragonSDK) OptionsMap() map[string]any {
...
func (sdk *DataDragonSDK) GetUtility() *Utility {
	return CopyUtility(sdk.utility)
}

func (sdk *DataDragonSDK) GetRootCtx() *Context {
	return sdk.rootctx
}

func (sdk *DataDragonSDK) Prepare(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *DataDragonSDK) Direct(fetchargs map[string]any) (map[string]any, error) {
...
func (sdk *DataDragonSDK) opAllowed(op string) bool {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return strings.Contains(allowOp, op)
}

func (sdk *DataDragonSDK) opDenied(op string) map[string]any {
	allowOp, _ := vs.GetPath([]any{"allow", "op"}, sdk.options).(string)
	return map[string]any{
		"ok": false,
		"err": fmt.Errorf("DataDragonSDK: %s: operation not allowed by"+
...
// gate by passing it.
func (sdk *DataDragonSDK) rawRequest(fetchargs map[string]any) (map[string]any, error) {
	utility := sdk.utility
...
func (sdk *DataDragonSDK) Graphql(
	...
	if 0 < len(errors) {
		...
		res["ok"] = false
		res["err"] = fmt.Errorf("DataDragonSDK: graphql: %s", msg)
		res["graphql"] = errors
	}

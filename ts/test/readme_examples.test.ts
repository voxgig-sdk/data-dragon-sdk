...
//     (\`new DataDragonSDK(...)\` / \`DataDragonSDK.test(...)\` become
//     \`DataDragonSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"champion":{"test01":{"id":"test01"}},"data_champion":{"test01":{"id":"test01"}},"data_item":{"test01":{"id":"test01"}},"data_rune":{"test01":{"id":"test01"}},"dragontail_versiontgz":{"test01":{"id":"test01"}},"item":{"test01":{"id":"test01"}},"region":{"test01":{"id":"test01"}},"version":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DataDragonSDK(...)\` and any
// \`DataDragonSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DataDragonSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DataDragonSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)

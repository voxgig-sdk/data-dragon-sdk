...
describe('DragontailVersiontgzDirect', async () => {
  ...
  test('direct-load-dragontail_versiontgz', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-dragontail_versiontgz', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DATA_DRAGON_TEST_DRAGONTAIL_VERSIONTGZ_ENTID']

...
describe('DataChampionDirect', async () => {
  ...
  test('direct-load-data_champion', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-data_champion', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['DATA_DRAGON_TEST_DATA_CHAMPION_ENTID']

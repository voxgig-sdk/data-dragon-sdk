...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.DATADRAGON_TEST_LIVE ||
    'TRUE' === process.env.DATADRAGON_TEST_OVERRIDE
  ) {
  ...
  m.DATADRAGON_TEST_EXPLAIN = process.env.DATADRAGON_TEST_EXPLAIN || m.DATADRAGON_TEST_EXPLAIN

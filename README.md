...
Data Dragon is a static data API providing information and images related to champions, runes, and items in League of Legends.

Learn more about Data Dragon API at [riot-api-libraries.readthedocs.io/en/latest/ddragon.html](https://riot-api-libraries.readthedocs.io/en/latest/ddragon.html).
...
This SDK exposes the API as a small set of **semantic entities** — Champion, DataChampion, DataItem, DataRune, DragontailVersiontgz, Item, Region and Version — that you
...
// Load a specific champion (returns a Champion)
const champion = await client.Champion().load({
...
champion, err := client.Champion(nil).Load(
    map[string]any{"version": "example_version", "id": "example_id"}, nil,
)
if err != nil {
    panic(err)
}
...
-- Load a specific champion
local champion, err = client:Champion():load({ id = "example_id", version = "example_version" })
print(champion)
```
